...
### Tasker LLM model runtime config
The LLM model for both the actionability classifier and the task generator is runtime-configurable via `generation_settings.llm_model` (admin UI: `PUT /admin/generation-settings`, field `llmModel`). Empty string falls back to the `llm.model` from the service config. The chat model is no longer a single startup instance: `llm.NewSettingsChatModelProvider` (`services/llm/model_provider.go`) reads the model name from the cached `generationSettingsService` and lazily rebuilds the OpenRouter `ChatModel` only when the name changes (construction does no network I/O, so this is cheap). The API key still comes from config. Changes propagate within the 30s settings cache TTL — no restart needed.
